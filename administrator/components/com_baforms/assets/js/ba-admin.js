...
function prepareFields(key, field)
{
    ...
    if (field) {
        ...
        if ('title' in app.items[id] && key != 'acceptance') {
            app.items[id].title = formsLanguage['ENTER_FIELD_TITLE'];
            content.querySelector('.ba-input-label-wrapper').textContent = formsLanguage['ENTER_FIELD_TITLE'];
        }
        if ('confirm' in app.items[id]) {
            app.items[id].confirm.title = formsLanguage['CONFIRM_EMAIL'];
            app.items[id]['confirm-password'].title = formsLanguage['CONFIRM_PASSWORD'];
        }
    }
...
app.google =  {
    ...
    setDriveIntegration: function(){
        ...
        if (app.google.drive.client_id && app.google.drive.client_secret && integrations.google_drive.key.accessToken) {
            app.fetch('index.php?option=com_baforms&view=form&task=form.getDriveFolders', {
        ...
        $f('.authenticate-google-drive').on('input', function(){
            ...
            this.delay = setTimeout(function(){
                ...
                app.fetch('index.php?option=com_baforms&view=form&task=form.createDriveToken', {
                    client_id: app.google.drive.client_id,
                    client_secret: app.google.drive.client_secret,
    ...
    setSheetsIntegration: function(){
        ...
        if (app.google.sheets.client_id && app.google.sheets.client_secret && integrations.google_sheets.key.accessToken) {
            app.fetch('index.php?option=com_baforms&view=form&task=form.getSpreadSheets', {
                client_id: app.google.sheets.client_id,
                client_secret: app.google.sheets.client_secret,
        ...
        $f('.authenticate-google-sheets').on('input', function(){
            ...
            this.delay = setTimeout(function(){
                ...
                $f('.google-sheet-worksheets, .google-sheet-spreadsheets').val('');
                $f('.google-sheets-fields .ba-settings-item').remove();
                app.fetch('index.php?option=com_baforms&view=form&task=form.createSheetsToken', {
                    client_id: app.google.sheets.client_id,
                    client_secret: app.google.sheets.client_secret,
                    code: $this.value.trim(),
                }).then(function(text){
...
app.loadCodeMirror = function(callback){
    var file = document.createElement('link'),
        ...
        html = document.getElementById('custom-html-editor'),
        cssFlag = false,
        jsFlag = false;
    file.rel = 'stylesheet';
    ...
    file.onload = function(){
        ...
        file.onload = function(){
            ...
            var input = document.getElementById('code-css-value'),
                style = document.querySelector('#custom-css-editor > style');
            app.codeCss.setValue(input.value);
            cssFlag = true;
            if (cssFlag && jsFlag && callback == 'showCodeEditor') {
                app.codemirror = true;
                app.showCodeEditor();
            }
        ...
        file.onload = function(){
            ...
            var input = document.getElementById('code-js-value');
...
app.showLogin = function(onMessage){
    ...
    if (login) {
        ...
        showNotice(formsLanguage['AWESOME_FEATURE_AVAILABLE_IN_PRO'], 'ba-alert');
    }
}
...
app.setMinicolors = function(){
    ...
    $f('.variables-color-picker').minicolors({
        ...
        change: function(hex, opacity) {
            var rgba = $f(this).minicolors('rgbaString');
            fontBtn.value = hex;
...
app.prepareSave = function(){
    $f('.forms-save').on('click', function(){
        ...
        let title = document.querySelector('.ba-form-title'),
            ...
            url = 'index.php?option=com_baforms&task=form.formsSave';
        if (!obj.form.title) {
            this.dataset.click = '';
...
function prepareInputType()
{
    if (!app.edit['confirm-password']) {
        app.edit['confirm-password'] = {
            "enable": false,
            "title": formsLanguage['CONFIRM_PASSWORD'],
            "description": "",
            "placeholder": "",
            "icon": "",
            "default":""
...
function setFieldValues(modal)
{
    ...
    if (app.edit.items) {
        ...
        for (let ind in app.edit.items) {
            container.append(getSortingItemHTML(app.edit.items[ind], ind));
        }
    }
...
app.showHtmlEditor = function(){
    if (app.codemirror) {
        ...
        app.loadCodeMirror('showHtmlEditor');
    }
}
...
app.showFieldDesign = function(){
    ...
    if (app.edit.design) {
        $f('#calculation-field-settings-dialog .field-design-options').hide();
    } else {
        $f('#calculation-field-settings-dialog .field-design-options').css('display', '');
    }
}
...
function installTemplate()
{
    if (app.formsTemplates.length > 0) {
        ...
        showNotice(formsLanguage['TEMPLATES_INSTALLED']);
    }
}
    ...
    if (payments.indexOf(ind) != -1) {
        ...
        if (flag) {
            ...
            if (ind == 'authorize') {
                ...
                title = '2Checkout';
            } else if (ind == 'paypal' || ind == 'paypal_sdk') {
                title = 'PayPal';
            } else if (ind == 'payfast') {
                title = 'PayFast';
...
function setIntegrationValues(ind)
{
    ...
    $f('.integration-options[data-group="'+ind+'"] [data-key]').each(function(){
        if (this.dataset.key == 'key') {
            ...
            $f(this).val(obj[this.dataset.key]);
        }
    ...
    $f.ajax({
        ...
        success: function(msg){
            ...
            if (integrations.zoho_crm.key.layout_id) {
                drawZohoCRMFields(obj[integrations.zoho_crm.key.layout_id].fields, true);
            }
        }
    });
}
...
function drawZohoCRMFields(fields, checkActive)
{
    ...
    if (checkActive) {
        ...
        checkIntegrationsActiveState('zoho_crm');
    }
}
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    $f('body').on('mousedown', function(event){
        ...
        if (event.target && (event.target.classList.contains('close-all-modals') || event.target.closest('.close-all-modals'))) {
            $f('.modal-backdrop').trigger('click');
        }
    ...
    $f('select.ba-units-select').on('change', function(){
        ...
        if (this.dataset.option == 'letter-spacing') {
            ...
            input.val(1);
        } else if (this.value == 'px' && this.dataset.option != 'width-value' && this.dataset.option != 'width') {
            input.val(14);
        } else {
    ...
    for (let ind in integrations) {
        ...
        } else if (ind == 'zoho_crm' && integrations.zoho_crm.key.grant_token) {
            getZohoCRMFields();
        } else if (ind == 'acymailing') {
            getAcymailingFields();
