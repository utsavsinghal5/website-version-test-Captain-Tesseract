...
abstract class baformsHelper 
{
    ...
    {
        $str = JFile::read(JPATH_ROOT.'/components/com_baforms/libraries/custom-payment-gateway/custom-payment-gateway.xml');
        $data = array();
    ...
    public static function checkIntegration($type)
    {
        switch ($type) {
            case 'redsys':
                ...
                break;
            case 'google_sheets':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/google-sheets';
                break;
            case 'google_drive':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/google-drive';
                break;
            case 'pdf_submissions':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/pdf-submissions';
                break;
            case 'campaign_monitor':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/campaign-monitor';
                break;
            case 'zoho_crm':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/zoho-crm';
                break;
        }
        $className = !JFolder::exists($path) ? ' require-library' : '';
    ...
    public static function setZohoCRMTempData($zoho)
    {
        $db = JFactory::getDbo();
    ...
    public static function getIntegrations($id)
    {
        ...
        if (empty($data->zoho_crm)) {
            $data->zoho_crm = self::addIntegration('zoho_crm', '{"client_id":"","client_secret":"","email":"","host":"","grant_token":""}');
            self::addIntegration('zoho_temp', '{}');
            self::addIntegration('zoho_oauthtokens', '{}');
        }
        $query = $db->getQuery(true)
            ->select('id, telegram_token, acym_lists, acym_fields_map, mailchimp_list_id, mailchimp_fields_map, google_drive,
                google_sheets, pdf_submissions, campaign_monitor_fields, getresponse_fields, zoho_crm_fields, activecampaign_fields')
    ...
    public static function getFormsLanguage()
    {
        ...
        $shortDate = array('JAN' => 'January 2019 ', 'FEB' => 'February 2019', 'MAR' => 'March 2019', 'APR' => 'April 2019',
            'MAY' => 'May 2019', 'JUN' => 'June 2019', 'JUL' => 'July 2019', 'AUG' => 'August 2019', 'SEP' => 'September 2019',
            'OCT' => 'October 2019', 'NOV' => 'November 2019', 'DEC' => 'December 2019');
