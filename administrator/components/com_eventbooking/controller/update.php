...
class EventbookingControllerUpdate extends RADController
{
	...
	public function update()
	{
		...
		{
			$sql = "ALTER TABLE  `#__eb_events` ADD `enable_sms_reminder` tinyint(3) UNSIGNED DEFAULT 1;";
			$db->setQuery($sql)
				->execute();
		}
		...
		if (JFile::exists(JPATH_ADMINISTRATOR . '/manifests/packages/pkg_eventbooking.xml'))
		{
			...
			if (!is_null($manifest))
			{
				...
				if ($eid && $manifest->updateservers)
				{
					...
					JFactory::getApplication()->triggerEvent('onExtensionAfterUpdate', ['installer' => $tmpInstaller, 'eid' => $eid]);
				}
			}
		}
