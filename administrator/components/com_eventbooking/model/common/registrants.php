...
class EventbookingModelCommonRegistrants extends RADModelList
{
	...
	protected function buildQueryColumns(JDatabaseQuery $query)
	{
		...
		$query->select('tbl.*, ev.title, ev.event_date, ev.event_end_date, ev.ticket_prefix, ev.main_category_id, ev.custom_fields, ev.activate_certificate_feature, u.username, cp.code AS coupon_code, cp.id AS coupon_id')
	...
	protected function buildQueryWhere(JDatabaseQuery $query)
	{
		...
		if ($this->state->filter_from_date && $this->state->filter_from_date != $db->getNullDate())
		{
			...
			try
			{
				$date = DateTime::createFromFormat($dateFormat, $fromDate, new DateTimeZone(JFactory::getConfig()->get('offset')));
		...
		if ($this->state->filter_to_date && $this->state->filter_to_date != $db->getNullDate())
		{
			...
			try
			{
				$date = DateTime::createFromFormat($dateFormat, $toDate, new DateTimeZone(JFactory::getConfig()->get('offset')));
