    ...
    function createDocument()
    {
        ...
        function listenMessage(event)
        {
            if (event.origin == location.origin) {
                ...
                jQuery('#ba-media-manager').load(url, function(){
                    window.parent.showNotice(message);
                    createDocument();
                });
            }
        }
        ...
        function UploadFiles()
        {
            if (files.length > 0) {
                ...
                if (XHR.upload && file.size <= 66000000) {
                    ...
                    if (total > max) {
                        window.parent.showNotice(jQuery('#post-max-error').val());
                    } else {
                        var upload = jQuery('#uploading-media').val(),
                            str = '';
                        upload = JSON.parse(upload);
                ...
                window.parent.showNotice(jQuery('#success-upload').val());
            }
        }
        ...
        jQuery('#file-upload-form [type="file"]').off('change').on('change', function(event){
            if (this.files.length > 0) {
                ...
                if (files.length == 0) {
                    window.parent.showNotice(window.parent.gridboxLanguage['NOT_SUPPORTED_FILE'], 'ba-alert');
                } else {
            ...
            drop: function(event, ui) {
                ...
                jQuery.ajax({
                    ...
                    success: function(msg) {
                        msg = JSON.parse(msg);
                        window.parent.showNotice(msg.message)
                    }
                });
            }
        });
        ...
        function addActive()
        {
            ...
            if (imageChecked && !window.parent.document.body.classList.contains('view-dashboard')) {
