            ...
            foreach ($menu_items as $item) {
                if ($item->published == 1 || $item->published == 0) {
                    ...
                    $array = array('menutype' => $menu->menutype, 'title' => $item->title,
                        ...
                        'level' => $item->level, 'component_id' => $com_id, 'access' => $item->access,
                        'template_style_id' => 0, 'language' => $item->language, 'params' => $item->params,
        ...
        while ($pos = strpos($obj->html, 'data-megamenu=', $pos)) {
            ...
            if ($match[0] && $match[0][1] - $pos < 20) {
                ...
                $replace = preg_replace_callback('/\d+/', function($matchData) use ($main_menu){
                        return isset($main_menu[$matchData[0]]) ? $main_menu[$matchData[0]] : $matchData[0];
                    }, $substr);
                if ($substr != $replace) {
