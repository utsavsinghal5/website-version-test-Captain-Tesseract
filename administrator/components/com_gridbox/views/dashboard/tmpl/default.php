...
<script src="components/com_gridbox/assets/js/sortable.js" type="text/javascript"></script>
...
<?php
                                        ...
                                        $dates = array('d' => $d, 'w' => $w, 'm' => JText::_('MONTHLY').', '.date('Y'),
...
<?php
                                    foreach ($this->pages as $page) {
                                        if (!empty($page->page_category) && !empty($page->intro_image)) {
                                            $introStr = '<span class="post-intro-image" style="background-image: url(';
                                            if (strpos($page->intro_image, 'balbooa.com') === false) {
                                                $introStr .= JUri::root().$page->intro_image;
                                            } else {
                                                $introStr .= $page->intro_image;
                                            }
                                            $introStr .= ');"></span>';
                                        } else {
...
<?php
                                            if ($imageFlag) {
?>
                                                <span class="recent-file-image" style="background-image: url(<?php echo $url; ?>);"></span>
<?php
                                            } else {
...
<?php
                                    ...
                                    foreach ($this->comments as $comment) {
                                        ...
                                        if (empty($avatar)) {
                                            $avatar = gridboxHelper::getGravatarImage($comment->email);
                                        }
...
<?php
                                    ...
                                    foreach ($this->reviews as $review) {
                                        ...
                                        if (empty($avatar)) {
                                            $avatar = gridboxHelper::getReviewsGravatarImage($review->email);
                                        }
                                        $date = date('Y-m-d', $timestamp);
?>
                                        <tr>
