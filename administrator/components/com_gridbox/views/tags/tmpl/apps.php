    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        for (var i = 0; i < items.length; i++) {
            items[i].addEventListener('click', function(event){
                ...
                var url = 'index.php?option=com_gridbox&view=tags&layout=modal&tmpl=component&id='+this.dataset.id,
                ...
                window.parent.document.querySelector('#jform_request_tag_name').value = '';
                window.parent.document.querySelector('#jform_request_tag_id').value = '';
                window.parent.jQuery('#gridbox_app_modal').modal('hide');
                window.parent.document.querySelector('#gridbox-category-modal .modal-body').innerHTML = str;
                url = 'index.php?option=com_gridbox&view=blog&layout=tag&app='+this.dataset.id;
                window.parent.document.querySelector('#jform_link').value = url;
            });
        }
    });
</script>
