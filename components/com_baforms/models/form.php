...
class baformsModelForm extends JModelItem
{
    ...
    public function sendMessage($data, $btn, $id)
    {
        ...
        if ($submit->options->notifications->enable) {
            ...
            if ($submit->options->notifications->email == 'customer-email' && !empty($userEmail)) {
                $sender = array($userEmail, '');
            } else if ($submit->options->notifications->email == 'custom' && !empty($submit->options->notifications->{'custom-email'})) {
                $sender = array($submit->options->notifications->{'custom-email'}, '');
            }
        ...
        if ($submit->options->reply->enable && !empty($userEmail)) {
            ...
            $sender = array($config->get('mailfrom'), $config->get('fromname'));
            if (!empty($submit->options->reply->email) && !empty($submit->options->reply->{'custom-email'})) {
                $sender = array($submit->options->reply->{'custom-email'}, '');
            }
...
?>
        <form id="payment-form" action="<?php echo $url; ?>" method="post">
            ...
            <input type="hidden" name="shopFailUrl" value="<?php echo $yandex->return_url; ?>">
            <input type="hidden" name="orderDetails" value="<?php echo $description; ?>">
        </form>
        <script type="text/javascript">
            document.getElementById('payment-form').submit();
        </script>
