...
class EasyBlogViewEntry extends EasyBlogView
{
	public function display( $tmpl = null )
	{
		...
		$pageTitle = EB::getPageTitle($this->config->get('main_title'));
	    $this->doc->setTitle( $post->title . $pageTitle );
		$this->doc->setName($post->getPermalink());

		// Fix phoca pdf plugin.
		if (method_exists($this->doc, 'setArticleText')) {
			$this->doc->setArticleText($blogHtml);
		}
	...
	public function isProtected(EasyBlogPost &$blog)
	{
		...
		$blog->creator 	= $blogger;

		$this->set('blogger', $blog->creator);
