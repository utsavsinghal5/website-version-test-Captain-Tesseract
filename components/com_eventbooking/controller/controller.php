...
class EventbookingController extends RADController
{
	...
	public function download_file()
	{
		...
		{
			$this->app->enqueueMessage(JText::_('File does not exist'), 'error');
	...
	public function validate_email()
	{
		...
		if (!isset($arrayToJs[1]))
		{
			...
			if ($domains)
			{
				...
				{
					$arrayToJs[1] = false;
					$arrayToJs[2] = JText::sprintf('JGLOBAL_EMAIL_DOMAIN_NOT_ALLOWED', $emailDomain);
				}
			}
		}
	...
	public function get_states()
	{
		...
		echo JHtml::_('select.genericlist', $options, $fieldName, ' class="uk-select input-large ' . $class . '" id="' . $fieldName . '"', 'value', 'text',
			$stateName);
