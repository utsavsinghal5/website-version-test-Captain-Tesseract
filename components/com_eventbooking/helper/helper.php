...
class EventbookingHelper
{
	...
	{
		return file_exists(JPATH_LIBRARIES . '/easylib/vendor/autoload.php') && JPluginHelper::isEnabled('content', 'ebstipeasyimage');
	}
	...
	public static function validateRules()
	{
		...
		$dateFormat = $config->date_field_format ? $config->date_field_format : '%Y-%m-%d';
		$dateFormat = str_replace('%', '', $dateFormat);
		$dateNow    = JHtml::_('date', 'now', $dateFormat);
	 ...
	 * @return TCPDF
	 */
	public static function getTCPDF($title, $pageOrientation = PDF_PAGE_ORIENTATION, $pageFormat = PDF_PAGE_FORMAT)
	{
	...
	public static function generateInvoicePDF($row)
	{
		...
		$pdf = EventbookingHelper::callOverridableHelperMethod('Helper', 'getTCPDF', ['Invoice']);
		...
		$invoiceOutput = EventbookingHelper::callOverridableHelperMethod('Html', 'processConditionalText', [$invoiceOutput]);
	...
	public static function generateCertificates($rows, $config)
	{
		...
		$pdf    = EventbookingHelper::callOverridableHelperMethod('Helper', 'getTCPDF', ['Certificate', $config->get('certificate_page_orientation', PDF_PAGE_ORIENTATION), $config->get('certificate_page_format', PDF_PAGE_FORMAT)]);
		$db     = JFactory::getDbo();
		$query  = $db->getQuery(true);
		$events = [];
