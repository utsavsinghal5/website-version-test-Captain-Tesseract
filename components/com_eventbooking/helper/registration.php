...
class EventbookingHelperRegistration
{
	...
	public static function calculateIndividualRegistrationFees($event, $form, $data, $config, $paymentMethod = null)
	{
		...
		list($vatNumberValid, $taxRate, $showVatNumberField) = static::calculateRegistrationTaxRate($event, $form, $data, $config);
	...
	public static function getFormData($rowFields, $eventId, $userId)
	{
		...
		if ($userId)
		{
			...
			$results = JFactory::getApplication()->triggerEvent('onGetProfileData', [$userId, $mappings]);
	...
	public static function canRefundRegistrant($row)
	{
		if ($row
			...
			&& $row->transaction_id
			&& !$row->refunded
			&& JFactory::getUser()->authorise('core.admin', 'com_eventbooking'))
		{
	...
	public static function calculateRegistrationTaxRate($event, $form, $data, $config)
	{
		...
		// If you want to hide VAT Number field for home country, then un-comment the lines below
