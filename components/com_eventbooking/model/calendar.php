...
class EventbookingModelCalendar extends EventbookingModelCommoncalendar
{
	...
	public function __construct($config = [])
	{
		...
		$this->params = EventbookingHelper::getViewParams(JFactory::getApplication()->getMenu()->getActive(), ['calendar']);
	}
	...
	public function getData()
	{
		...
		{
			$this->params = EventbookingHelper::getViewParams(JFactory::getApplication()->getMenu()->getItem($this->state->mini_calendar_item_id), ['calendar']);
		}
		...
		{
			$query->where("((a.event_date BETWEEN $startDate AND $endDate) OR (MONTH(a.event_end_date) = $month AND YEAR(a.event_end_date) = $year ))");
		}
		else
		{
	...
	public function getEventsByWeek()
	{
		...
		$weekStartDate = JFactory::getDate($this->state->date, JFactory::getConfig()->get('offset'));
		$activeDate    = JFactory::getDate('now', JFactory::getConfig()->get('offset'));
