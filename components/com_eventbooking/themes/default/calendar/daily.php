...
JFactory::getDocument()->addScriptOptions('dailyCalendarUrl', JRoute::_('index.php?option=com_eventbooking&view=calendar&layout=daily&Itemid=' . $this->Itemid . '&day=', false));

EventbookingHelperHtml::addOverridableScript('media/com_eventbooking/js/site-calendar-daily.min.js');

JText::script('EB_PLEASE_CHOOSE_DATE', true);
...
<div style="width: 100%;" class="topmenu_calendar">
	<div class="left_calendar">
		...
		<?php echo JHtml::_('calendar', JFactory::getApplication()->input->getString('day', ''),'date', 'date', '%Y-%m-%d'); ?>
		<input type="button" class="btn" value="<?php echo JText::_('EB_GO'); ?>" onclick="gotoDate();" />
	</div>
	<?php
	...
	<tr class="tablec">
		<td class="previousday">
			<a href="<?php echo JRoute::_("index.php?option=com_eventbooking&view=calendar&layout=daily&day=".date('Y-m-d',strtotime("-1 day", strtotime($this->day)))."&Itemid=$this->Itemid");?>" rel="nofollow">
                <i class="<?php echo $angleDoubleLeft; ?> eb-calendar-navigation" title="<?php echo JText::_('EB_PREVIOUS_DAY')?>"></i>
			</a>
		</td>
		...
		<td class="nextday">
			<a href="<?php echo JRoute::_("index.php?option=com_eventbooking&view=calendar&layout=daily&day=".date('Y-m-d',strtotime("+1 day", strtotime($this->day)))."&Itemid=$this->Itemid");?>" rel="nofollow">
                <i class="<?php echo $angleDoubleRight; ?> eb-calendar-navigation" title="<?php echo JText::_('EB_NEXT_DAY')?>"></i>
			</a>
		</td>
	</tr>
</table>
