...
trait EventbookingViewEvent
{
	...
	public function buildFormData($item, $categories, $locations)
	{
		...
		$this->lists['main_category_id'] = JHtml::_('select.genericlist', $options, 'main_category_id', [
		...
		$this->lists['free_event_registration_status'] = JHtml::_('select.genericlist', $options, 'free_event_registration_status', '', 'value', 'text', $item->free_event_registration_status);
		...
		$options[] = JHtml::_('select.option', 3, JText::_('EB_MONTHLY_BY_DAYS'));
		$options[] = JHtml::_('select.option', 4, JText::_('EB_MONTHLY_BY_WEEKDAY'));
		...
		$results = JFactory::getApplication()->triggerEvent('onEditEvent', [$item]);
