...
class EventbookingViewRegisterHtml extends EventbookingViewRegisterBase
{
	...
	public function display()
	{
		...
		JText::script('EB_INVALID_VATNUMBER', true);
		...
		if (!$pageTitle)
		{
			...
			{
				$pageTitle = JText::_('EB_INDIVIDUAL_REGISTRATION_PAGE_TITLE');
			}
			elseif ($layout == 'group' && $language->hasKey('EB_GROUP_REGISTRATION_PAGE_TITLE'))
			{
				$pageTitle = JText::_('EB_GROUP_REGISTRATION_PAGE_TITLE');
			}
			else
			{
			...
			$pageTitle = str_replace('[EVENT_DATE]', JHtml::_('date', $event->event_date, $config->event_date_format, null), $pageTitle);
		...
		if (!EventbookingHelper::callOverridableHelperMethod('Registration', 'acceptRegistration', [$event]))
		{
			...
			if ($waitingList && $user->id)
			{
				...
				if ($preventDuplicateRegistration && $user->id > 0)
				{
					...
					if ($total)
					{
						$app->enqueueMessage(JText::_('EB_JOINED_WAITING_LIST_ALREADY'), 'error');
						$app->redirect(JRoute::_(EventbookingHelperRoute::getEventRoute($event->id, $event->main_category_id), false));
					}
				}
			}
