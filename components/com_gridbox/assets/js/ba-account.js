...
document.addEventListener('DOMContentLoaded', function(){
    ...
    let countries = null,
        ...
        alertTooltip = {
            ...
            test: function($this, data){
                ...
                if (($this.name == 'name' || $this.name == 'email1' || $this.name == 'email2') && !data[$this.name]) {
                    alert = true;
                    alertTooltip.toggle(true, $this, $this.closest('.ba-account-profile-fields'), 'THIS_FIELD_REQUIRED');
                } else if (($this.name == 'email1' || $this.name == 'email2') && data.email1 != data.email2) {
                    alert = true;
                    key = 'EMAILS_ENTERED_NOT_MATCH';
                } else if (($this.name == 'password1' || $this.name == 'password2') && data.password1 != data.password2) {
                    alert = true;
    ...
    function checkEmptyWishlist()
    {
        if (!document.querySelector('#ba-my-account-wishlist .ba-wishlist-product-row')) {
            let html = '<div class="ba-empty-cart-products"><i class="zmdi zmdi-favorite"></i>'
    ...
    $g('#ba-my-account-profile input').on('focus', function(){
        alertTooltip.remove(this);
    });
    $g('.save-user-profile-data').on('click', function(){
        let data = {},
            alert = false;
        $g('#ba-my-account-profile input[name]').each(function(){
    ...
    $g('#ba-my-account-orders .ba-account-tr[data-id]').on('click', function(){
        app.fetch(JUri+'index.php?option=com_gridbox&view=editor&task=account.getOrder', {
            ...
            order.products.forEach(function(product){
                ...
                if (product.image) {
                    let src = (product.image.indexOf('balbooa.com') == -1 ? JUri : '')+product.image;
    ...
    $g('.ba-account-order-icon-wrapper i').on('click', function(){
        ...
        iframe.src = JUri+'index.php?option=com_gridbox&view=account&layout='+
            this.dataset.layout+'&tmpl=component&id='+this.dataset.id
    });
    $g('.save-user-customer-info').on('click', function(){
        let data = {};
        document.querySelectorAll('#ba-my-account-billing-details [name]').forEach(function(input){
