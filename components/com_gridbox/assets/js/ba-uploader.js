...
jQuery(document).ready(function(){
    jQuery('body').addClass('component');
    ...
    function createDocument()
    {
        jQuery('a[data-href]').each(function(){
            this.href = 'index.php?option=com_gridbox&view=uploader&'+this.dataset.href;
        });
        ...
        function listenMessage(event)
        {
            if (event.origin == location.origin) {
                ...
                jQuery('#ba-media-manager').load(url, function(){
                    window.parent.app.showNotice(message);
                    createDocument();
                });
            }
        }
        ...
        var notification = window.parent.document.getElementById('ba-notification');
        ...
        function UploadFiles()
        {
            if (files.length > 0) {
                var file = files.pop(),
                    ...
                    url = "index.php?option=com_gridbox&&task=uploader.uploadAjax&file="+file.name;
                if (XHR.upload && file.size <= 66000000) {
                    var upload = jQuery('#uploading-media').val(),
                        str = '';
                    upload = JSON.parse(upload);
                    str += upload.const+'<img src="'+
                        window.parent.JUri+'components/com_gridbox/assets/images/reload.svg"></img>';
                    ...
                    if (total > max) {
                        window.parent.app.showNotice(jQuery('#post-max-error').val());
                    } else {
                        var upload = jQuery('#uploading-media').val(),
                            str = '';
                        upload = JSON.parse(upload);
                ...
                var sBackdrop = window.parent.document.getElementsByClassName('saving-backdrop')[0];
                sBackdrop.className += ' animation-out';
                setTimeout(function(){
                    window.parent.document.getElementsByTagName('body')[0].removeChild(sBackdrop);
                }, 300);                
                window.parent.app.showNotice(jQuery('#success-upload').val());
            }
        }
        ...
        jQuery('#file-upload-form [type="file"]').off('change').on('change', function(event){
            if (this.files.length > 0) {
                ...
                if (files.length == 0) {
                    window.parent.app.showNotice(window.parent.gridboxLanguage['NOT_SUPPORTED_FILE'], 'ba-alert');
                } else {
        ...
        jQuery(".ba-folder-tree li a, tbody tr:not(.ba-images)").droppable({
            ...
            drop: function(event, ui) {
                ...
                jQuery.ajax({
                    type:"POST",
                    dataType:'text',
                    url:"index.php?option=com_gridbox&view=uploader&task=uploader.moveTo",
                    data:{
                        'ba_image' : path,
                        'ba_folder' : move
                    },
                    success: function(msg) {
                        msg = JSON.parse(msg);
                        window.parent.app.showNotice(msg.message)
                    }
                });
            }
        });
        ...
        jQuery('#apply-rename').on('click', function(event){
            ...
            jQuery.ajax({
                type : "POST",
                dataType : 'text',
                url : "index.php?option=com_gridbox&view=uploader&task=uploader.renameTarget",
                data : {
                    'ba_target' : target,
                    'ba_name' : name
                },
        ...
        jQuery('.edit-image').on('mousedown', function(){
            var str = currentItem.find('.ba-obj').val(),
                obj = JSON.parse(str);
            window.parent.app.itemDelete = obj;
            window.parent.app.checkModule('photoEditor');
        });
        ...
        jQuery('.apply-move').on('click', function(event){
            ...
            jQuery.ajax({
                type:"POST",
                dataType:'text',
                url:"index.php?option=com_gridbox&view=uploader&task=uploader.moveTarget",
                data:{
                    'ba_target' : target,
                    'ba_path' : path,
                    'ba_flag' : moveTo
        ...
        jQuery('#apply-delete').on('click', function(event){
            ...
            if (deleteMode == 'default') {
                ...
                jQuery.ajax({
                    type:"POST",
                    dataType:'text',
                    url:"index.php?option=com_gridbox&view=uploader&task=uploader.deleteTarget",
                    data:{
                        'ba_target' : target
                    },
                    success : function(msg) {
            ...
            jQuery('.select-item').each(function(){
                ...
                if (jQuery(this).closest('.ba-images').hasClass('ba-images') && jQuery(this).prop('checked')) {
                    ...
                    if (document.body.classList.contains('photo-editor-enabled')) {
                        var ext = jQuery(this).closest('tr').attr('data-ext'),
                            imageTypes = new Array('jpg', 'png', 'jpeg', 'webp');
        ...
        function sendMessage()
        {
            ...
            if (msg.length > 0 || typeof(msg) == 'object') {
                if (document.body.classList.contains('photo-editor-enabled')) {
                    window.parent.$g('#uploader-modal').modal('hide');
                    window.parent.app.itemDelete = msg;
                    window.parent.app.checkModule('photoEditor');
                } else {
