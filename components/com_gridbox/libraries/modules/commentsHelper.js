...
app.getFacebookUserInfo = function(response){
    if (response.status === 'connected') {
        let request = '/'+response.authResponse.userID+'/?fields=id,name,email,picture';
        FB.api(request, 'GET', {}, function(data){
            let obj = {
                name: data.name,
                email: data.email,
...
app.getGoogleUserInfo = function(){
...
app.loginCommentSocial = function(data){
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task=comments.loginSocial",
        data: data,
...
app.getReviewById = function($this){
    ...
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task=reviews.getReviewById",
        data: {
            page: themeData.id,
            id: id[0],
            replyName: replyName,
        ...
        success: function(msg){
            ...
            wrapper.find('.comment-reply-form-wrapper, .ba-comment-reply-wrapper').find('.ba-comments-attachments-wrapper').remove();
...
app.getCommentsPatterns = function(controller, next, reviewID){
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task="+controller+".getCommentsPatterns",
        data: {
            id: themeData.id,
            next: next ? next : 1,
            'sort-by': $g('.ba-comments-total-count-wrapper select').val(),
        ...
        success: function(msg){
            ...
            parent.find('.ba-comments-total-count-wrapper').html(object.commentsCount).find('meta[itemprop="name"]').each(function(){
            ...
            parent.find('.comment-reply-form-wrapper, .ba-comment-reply-wrapper').find('.ba-comments-attachments-wrapper').remove();
            ...
            if (object.captcha == '') {
                ...
                parent.find('> .ba-comments-box-wrapper .ba-comment-message-wrapper .ba-comments-captcha-wrapper').each(function(){
...
app.getReviewsMoreReply = function(wrapper, limit){
    ...
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task=reviews.getReviewsMoreReply",
        data: {
            id: themeData.id,
            parent: id[0],
            replyName: replyName,
...
function initGoogleLogin()
{
    gapi.load('auth2', function() {
        gapi.auth2.init({
            client_id: commentsSocial.google
        }).then(function(googleAuth){
            app.googleAuth = googleAuth;
        }, function(obj){
            console.error(obj.details)
        });
    });
}
...
{
    VK.init({apiId: commentsSocial.vk});
}

if (window.commentsSocial && commentsSocial.facebook) {
    (function(d, s, id) {
...
if (window.commentsSocial && commentsSocial.google) {
    (function(d, s) {
        var js, fjs = d.getElementsByTagName(s)[0];
        js = d.createElement(s);
        js.src = "https://apis.google.com/js/platform.js?onload=initGoogleLogin";
        fjs.parentNode.insertBefore(js, fjs);
    }(document, 'script'));
}

if (window.commentsSocial && commentsSocial.vk) {
    (function(d, s) {
        var js, fjs = d.getElementsByTagName(s)[0];
        js = d.createElement(s);
...
app.getCommentsModeratorsContext = function(){
    if (!app.commentsModeratorsContext) {
        var div = document.createElement('div'),
            str = '<span class="comments-approve"><i class="zmdi zmdi-check"></i>'+gridboxLanguage['APPROVE']+'</span>'+
            '<span class="comments-spam"><i class="zmdi zmdi-alert-octagon"></i>'+gridboxLanguage['SPAM']+'</span>'+
            '<span class="comments-ban-user"><i class="zmdi zmdi-block"></i>'+gridboxLanguage['BAN_USER']+'</span>'+
            '<span class="comments-delete ba-group-element"><i class="zmdi zmdi-delete"></i>'+gridboxLanguage['DELETE']+'</span>';
        ...
        app.commentsModeratorsContext.on('click', '.comments-approve', function(){
            ...
            $g.ajax({
                type: "POST",
                dataType: 'text',
                url: "index.php?option=com_gridbox&task="+controller+".moderatorApprove",
                data: {
        ...
        app.commentsModeratorsContext.on('click', '.comments-spam', function(){
            ...
            $g.ajax({
                type: "POST",
                dataType: 'text',
                url: "index.php?option=com_gridbox&task="+controller+".moderatorSpam",
                data: {
                    id: app.shareComment.id.replace(/reviewID-|commentID-/, '')
                },
                complete: function(msg){
        ...
        app.commentsModeratorsContext.on('click', '.comments-ban-user', function(){
            ...
            $g.ajax({
                type: "POST",
                dataType: 'text',
                url: "index.php?option=com_gridbox&task="+controller+".moderatorBanUser",
                data: {
                    id: app.shareComment.id.replace(/reviewID-|commentID-/, '')
                },
                complete: function(msg){
...
app.getCommentsShareDialog = function(){
    if (!app.commentsShareDialog) {
        ...
        str += '<i class="zmdi zmdi-twitter twitter-share-comment"></i><i class="zmdi zmdi-facebook facebook-share-comment"></i>';
        str += '<i class="zmdi zmdi-vk vk-share-comment"></i><span><i class="zmdi zmdi-link copy-comment-link"></i><span class="ba-tooltip">';
    ...
    if (!app.commentsDeleteDialog) {
        ...
        app.commentsDeleteDialog.find('.apply-comment-delete').on('click', function(){
            ...
            $g.ajax({
                type: "POST",
                dataType: 'text',
                url: "index.php?option=com_gridbox&task="+controller+"."+task,
                data: {
                    id: id
                },
                complete: function(msg){
    ...
    if (!app.userLoginDialog) {
        ...
        app.userLoginDialog.find('.ba-user-login-action').on('click', function(){
            ...
            $g.ajax({
                type: "POST",
                dataType: 'text',
                url: "index.php?option=com_gridbox&task=comments.loginUser",
                data: data,
                complete: function(msg){
        ...
        app.guestLoginDialog.find('.ba-guest-login-action').on('click', function(){
            ...
            if (nameFlag && emailFlag) {
                $g.ajax({
                    type: "POST",
                    dataType: 'text',
                    url: "index.php?option=com_gridbox&task=comments.loginGuest",
                    data: data,
                    complete: function(msg){
                        app.setCommentsUser(msg.responseText);
                    }
...
function removeTmpAttachment($this)
{
    if ($this.dataset.id) {
        ...
        $g.ajax({
            type : "POST",
            dataType : 'text',
            url : "index.php?option=com_gridbox&task="+controller+".removeTmpAttachment",
            data: {
                id: $this.dataset.id,
...
function uploadCommentAttachmentFile(files, type, container)
{
    if (files.length) {
        ...
        xhr.open("POST", "index.php?option=com_gridbox&task="+controller+".uploadAttachmentFile", true);
        xhr.send(formData);
    } else {
...
function getCommentUser()
{
    ...
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task=comments.getCommentsUser",
        complete: function(msg){
            if (msg.responseText) {
                try {
...
$g('body').on('click', '.comment-report-user-comment', function(){
    ...
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task="+controller+".sendCommentReport",
        data: {
            id: id
        }
    });
...
$g('body').on('click', '.ba-comments-facebook-login', function(e){
    if (window.commentsSocial && commentsSocial.facebook && !app.commentsUser && window.FB) {
        FB.login(function(response){
            app.getFacebookUserInfo(response);
        });
    }
});
$g('body').on('click', '.ba-comments-google-login', function(e){
    if (window.commentsSocial && commentsSocial.google && !app.commentsUser && app.googleAuth) {
        app.googleAuth.signIn().then(function(response){
            app.getGoogleUserInfo();
        }, function(){});
    }
});
$g('body').on('click', '.ba-comments-vk-login', function(e){
    if (window.commentsSocial && commentsSocial.vk && !app.commentsUser && window.VK) {
...
$g('body').on('click', '.ba-submit-comment', function(e){
    ...
    if (allowSubmit) {
        ...
        $g.ajax({
            type: "POST",
            dataType: 'text',
            url: "index.php?option=com_gridbox&task="+controller+".sendCommentMesssage",
            data: data,
            complete: function(msg){
...
$g('body').on('click', '.ba-submit-cancel', function(){
    if (this.closest('.ba-leave-review-wrapper')) {
        ...
        $g(this).closest('.comment-data-wrapper').find('.ba-comment-xhr-attachment i.zmdi-delete').trigger('click');
    }
});
...
$g('body').on('click', '.ba-leave-review-btn', function(event){
    $g('.user-comment-edit-enable .ba-submit-cancel, .leave-review-enabled .ba-submit-cancel').trigger('click');
    $g('.comment-reply-form-wrapper').hide();
    $g(this).closest('.ba-comments-box-wrapper').addClass('leave-review-enabled').css('display', '')
        .find('.ba-comments-captcha-wrapper').each(function(){
        if (!this.querySelector('*')) {
            app.initCommentsRecaptcha(this);
        }
...
$g('body').on('click', '.comment-logout-action', function(event){
    ...
    $g.ajax({
        type: "POST",
        dataType: 'text',
        url: "index.php?option=com_gridbox&task=comments.logoutUser",
        complete: function(msg){
            app.reloadAllCommentsPatterns();
        }
    });
...
$g('body').on('click', '.comment-likes-action', function(event){
    ...
    $g.ajax({
        type : "POST",
        dataType : 'text',
        url : "index.php?option=com_gridbox&task="+controller+".setLikes",
        data: {
            id: id,
            action: this.dataset.action
        },
