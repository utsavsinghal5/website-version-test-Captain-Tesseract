...
app.customerInfoEditor = function(){
    ...
    modal.find('.slideshow-style-custom-select input[readonly]').val(gridboxLanguage['LABEL']);
...
function getCustomerInfoSortingHTML(obj, key)
{
    var disabled = obj.type == 'email' || obj.id == 1 || obj.type == 'country' ? 'disabled' : '',
        title = obj.type == 'acceptance' ? gridboxLanguage['ACCEPTANCE'] : (obj.title ? obj.title : obj.settings.placeholder),
...
$g('#customer-info-settings-dialog .sorting-container').on('click', 'i[data-action]', function(){
    ...
    if (this.dataset.action == 'edit') {
        ...
        modal.find('[data-key]').each(function(){
            ...
            if (this.type == 'hidden') {
                this.closest('.ba-custom-select').querySelector('input[type="text"]').value = gridboxLanguage[obj.type.toUpperCase()];
            }
        });
        modal.find('[data-settings]').each(function(){
            ...
            if (this.type == 'checkbox') {
                ...
                let range = $g(this).val(value).prev().val(value);
...
function setCustomerInfoType(type, selected)
{
    ...
    if (type == 'acceptance') {
        ...
        modal.find('input[data-key="invoice"]').prop('checked', false).closest('.ba-group-element').addClass('disabled');
        ...
        modal.find('input[data-key="invoice"]').closest('.ba-group-element').removeClass('disabled');
        modal.find('input[data-key="title"]').css('display', '');
        modal.find('textarea[data-settings="html"]').hide();
    } else {
        modal.find('input[data-key="required"], input[data-key="invoice"]').closest('.ba-group-element').removeClass('disabled');
        modal.find('input[data-key="title"]').css('display', '');
        modal.find('textarea[data-settings="html"]').hide();
    }
    if (type == 'country') {
    ...
    if ((type == 'dropdown' || type == 'country') && selected) {
        modal.find('input[data-settings="placeholder"]').val(gridboxLanguage['SELECT']);
    } else if (selected) {
...
$g('#apply-customer-info').on('click', function(){
    ...
    if (obj.type == 'dropdown' || obj.type == 'country') {
        obj.settings.placeholder = gridboxLanguage['SELECT'];
    }
    if (['dropdown', 'radio', 'checkbox'].indexOf(obj.type) != -1) {
        obj.settings.options.push(gridboxLanguage['VALUE']);
...
$g('#apply-customer-info-item').on('click', function(){
    ...
    if (obj.type == 'acceptance') {
        title = gridboxLanguage['ACCEPTANCE'];
    }
    ...
    app.editor.$g(app.selector+' .ba-checkout-form-fields:nth-child('+(this.dataset.key * 1 + 1)+')').each(function(){
        ...
        let $this = $g(this),
            wrapper = $this.find('.ba-checkout-form-field-wrapper').empty();
...
function createCustomerInfoField(wrapper, obj)
{
    ...
    if (['text', 'textarea', 'email'].indexOf(obj.type) != -1) {
        ...
        option.textContent = obj.settings.placeholder;
        element.append(option);
        for (let country in app.customerInfoEditor.countries) {
            option = document.createElement('option');
