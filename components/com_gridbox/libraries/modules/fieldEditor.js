...
app.fieldEditor = function(){
    ...
    $g('#field-settings-dialog .section-access-select input[type="hidden"]').val(app.edit.access);
    value = $g('#field-settings-dialog .section-access-select li[data-value="'+app.edit.access+'"]').text();
    $g('#field-settings-dialog .section-access-select input[readonly]').val($g.trim(value));
    $g('#field-settings-dialog .class-suffix').val(app.edit.suffix);
    value = app.getValue('margin', 'top');
    $g('#field-settings-dialog [data-group="margin"][data-option="top"]').val(value);
    value = app.getValue('margin', 'bottom');
    $g('#field-settings-dialog [data-group="margin"][data-option="bottom"]').val(value);
    ...
    $g('#field-settings-dialog [data-group="padding"][data-option="top"]').val(value);
    value = app.getValue('padding', 'right');
    $g('#field-settings-dialog [data-group="padding"][data-option="right"]').val(value);
    value = app.getValue('padding', 'bottom');
    $g('#field-settings-dialog [data-group="padding"][data-option="bottom"]').val(value);
    value = app.getValue('padding', 'left');
    $g('#field-settings-dialog [data-group="padding"][data-option="left"]').val(value);
    for (var key in app.edit.desktop.border) {
        var input = $g('#field-settings-dialog input[data-option="'+key+'"][data-group="border"]');
    ...
    $g('#field-settings-dialog .ba-style-custom-select input[readonly]').val(gridboxLanguage['LABEL']);
    showBaStyleDesign('title', document.querySelector('#field-settings-dialog .ba-style-custom-select'));
    value = app.getValue('icons', 'size');
    var range = $g('#field-settings-dialog [data-group="icons"][data-option="size"]').val(value).prev().val(value);
    setLinearWidth(range);
    ...
    if (app.edit.options.type == 'textarea') {
        ...
        $g('#field-settings-dialog .field-'+app.edit.options.type+'-type-options input[data-group="options"]').each(function(){
            ...
            if (this.type == 'hidden') {
                ...
                var range = $g(this).prev().val(this.value);
                setLinearWidth(range);
            }
        });
    ...
    if (obj.options.type == 'textarea') {
        ...
        $g('#group-field-item-dialog .field-'+obj.options.type+'-type-options input[data-element]').each(function(){
            ...
            if (this.dataset.element == 'source') {
                var value = obj.options[this.dataset.element] == 'desktop' ? gridboxLanguage['DESKTOP'] : gridboxLanguage['MEDIA_MANAGER'];
...
$g('#field-settings-dialog .field-admin-description').on('input', function(){
...
$g('#field-settings-dialog .fields-group-plugin-options .add-new-item .zmdi-plus-circle').on('click', function(){
    ...
    $g('#add-new-field-element-modal .field-element-type-select input[type="hidden"]').val('text').prev().val(gridboxLanguage['TEXT']);
    $g('#apply-new-field-element').removeClass('active-button').addClass('disable-button').attr('data-key', sortingList.length);
