...
function checkRecentPostsAppType(id, modal)
{
    ...
    ul.find('li[data-value="top_selling"]').remove();
    if (type == 'products') {
        ul.append('<li data-value="top_selling">'+gridboxLanguage['TOP_SELLING']+'</li>');
    }
    if (app.edit.sorting == 'top_selling' && type != 'products') {
        ...
        ul.closest('.ba-custom-select').each(function(){
            ...
            this.querySelector('input[type="text"]').value = gridboxLanguage['RECENT'];
        });
    }
    */
}
...
file.onload = function(){
    ...
    $g('#blog-post-editor-fields-options .field-sorting-wrapper .sorting-container').each(function(ind){
        $g(this).sortable({
            handle : '> .sorting-item .sorting-handle i',
            selector : '> .sorting-item',
    ...
    $g('#feature-box-settings-dialog .sorting-container').sortable({
        ...
        selector : '> .sorting-item',
        change : function(){
            var li = [].slice.call(app.editor.document.querySelectorAll(app.selector+' .ba-feature-box')),
                container = app.editor.$g(app.selector+' .ba-feature-box-wrapper'),
                ind = 0,
                slides = {};
    ...
    $g('#testimonials-settings-dialog .sorting-container').sortable({
        ...
        selector : '> .sorting-item',
        change : function(){
            var li = [].slice.call(app.editor.document.querySelectorAll(app.selector+' ul li.item')),
                container = app.editor.$g(app.selector+' .slideshow-content'),
                slides = {};
            $g('#testimonials-settings-dialog .sorting-item').each(function(ind){
                slides[ind + 1] = app.edit.slides[this.dataset.key * 1 + 1];
                container.append(li[this.dataset.key * 1]);
            });
    ...
    $g('#icon-list-settings-dialog .sorting-container').sortable({
        ...
        selector : '> .sorting-item',
        change : function(){
            var li = [].slice.call(app.editor.document.querySelectorAll(app.selector+' ul li')),
                container = app.editor.$g(app.selector+' ul'),
                list = {};
            $g('#icon-list-settings-dialog .sorting-item').each(function(ind){
                list[ind + 1] = app.edit.list[this.dataset.key * 1 + 1];
                container.append(li[this.dataset.key * 1]);
            });
            app.edit.list = list;
    ...
    $g('#content-slider-settings-dialog .sorting-container').sortable({
        ...
        change : function(){
            ...
            $g('#content-slider-settings-dialog .sorting-container .sorting-item').each(function(key){
                obj[key + 1] = app.edit.slides[this.dataset.key * 1 + 1];
...
function openPickerModal(modal, $this)
{
    if (!modal.hasClass('in')) {
        ...
        if (modal[0].id == 'category-list-fields-modal') {
            left = 360;
            top = rect.bottom - ((rect.bottom - rect.top) / 2) - 249;
            if (window.innerHeight - top < 500) {
                top = window.innerHeight - 500;
                bottom = (window.innerHeight - rect.bottom + rect.height / 2)+'px';
            } else if (top < 0) {
                top = 0;
                bottom = (500 - rect.bottom + rect.height / 2)+'px';
            }
        } else if (modal[0].id == 'product-badges-dialog') {
...
function showSlideshowDesign(search, select)
{
    ...
    if (search == 'field') {
        ...
        parent.find('.slideshow-normal-options').css('display', '').find('[data-subgroup]').attr('data-group', search);
    }
    ...
    if (app.edit.type == 'checkout-form' && search == 'title') {
        parent.find('.slideshow-typography-options').addClass('last-element-child');
    }
            ...
            if (ind == 'typography') {
                ...
                for (var key in value[ind]) {
                    ...
                    if (input.attr('data-type') == 'color') {
                        ...
                        input.prop('checked', value[ind][key]);
                    } else {
                        input.val(value[ind][key]);
...
function showBaStyleDesign(search, $this)
{
    ...
    switch (search) {
        case 'image' :
            ...
            parent.find('.ba-style-border-options').css('display', '').find('input[data-subgroup]').attr('data-group', search);
            break;
        case 'stars':
        ...
        case 'pagination' :
            if (app.edit.type != 'recent-posts') {
                ...
                parent.find('.ba-style-margin-options').show().find('[data-subgroup]').attr('data-group', search);
                parent.find('.ba-style-margin-options [data-type="reset"][data-subgroup="margin"]').attr('data-option', search);
                parent.find('.ba-style-button-options').show().find('input[data-subgroup]').attr('data-group', search);
    ...
    for (var ind in value) {
        if (typeof(value[ind]) == 'object') {
            if (ind != 'typography') {
                for (var key in value[ind]) {
                    ...
                    if (input.attr('data-type') == 'color') {
                        ...
                        input.each(function(){
                            this.classList[this.dataset.value == value[ind][key] ? 'add' : 'remove']('active');
                        });
                    } else {
                        input.val(value[ind][key]);
                        if (input.attr('type') == 'hidden') {
            ...
            if (input.attr('data-type') == 'color') {
                ...
                input.each(function(){
                    this.classList[this.dataset.value == value[ind] ? 'add' : 'remove']('active');
                });
            }
        }
    }
...
app.setValue = function(value, group, option, subgroup){
...
$g('input[data-type="color"]').on('click', function(){
    ...
    $g('#color-variables-dialog').css({
        left : rect.left - 285,
        top : rect.bottom - ((rect.bottom - rect.top) / 2) - 174
    }).removeClass('ba-right-position ba-top-position').modal().find('.nav-tabs li:last').css('display', '');
    if (this.closest('#yandex-maps-item-dialog')) {
        $g('#color-variables-dialog').find('.nav-tabs li:last').hide();
    }
...
function setOptionsState($this, state)
{
    $g($this).closest('.ba-settings-item').nextAll('.ba-settings-item').each(function(){
        var input =  $g(this).find('input[data-option]').attr('data-subgroup', state);
        value = app.getValue(input[0].dataset.group, input[0].dataset.option, input[0].dataset.subgroup);
        if (input[0].dataset.type == 'color') {
...
$g('.select-options-state').on('customAction', function(){
    var state = this.querySelector('input[type="hidden"]').value;
    setOptionsState(this, state);
});
...
$g('.open-category-list-fields').on('click', function(){
    ...
    if (app.edit.type == 'recent-posts' || app.edit.type == 'recent-posts-slider' || app.edit.type == 'fields-filter'
        || app.edit.type == 'google-maps-places'|| app.edit.type == 'event-calendar') {
        id = app.edit.app;
    }
    if (this.dataset.target == 'fields') {
            ...
            complete: function(msg){
                ...
                for (let i = 0; i < app.edit.fields.length; i++) {
                    let ind = app.edit.fields[i],
                        value = app.getValue('fields', ind),
                        label = ind == 'rating' ? gridboxLanguage['RATING'] : object[ind].label,
                        options = app.edit.type == 'fields-filter' ? null : JSON.parse(object[ind].options);
                    if ((app.edit.type == 'fields-filter' && !label) ||
                        (app.edit.type != 'fields-filter' && object[ind].product)) {
                        continue;
                    }
                    if (!label) {
                        label = options.label;
        ...
        app.fetch(JUri+'index.php?option=com_gridbox&task=store.getAppStoreFields', {
            ...
            for (let ind in json) {
                let value = app.getValue('store', ind);
...
$g('#category-list-fields-modal .category-list-fields-view-wrapper').on('change', 'input[type="checkbox"]', function(){
    app.setValue(this.checked, this.dataset.group, this.dataset.option);
    app.sectionRules();
    if (app.edit.type == 'recent-posts-slider' || app.edit.type == 'related-posts-slider'
        || app.edit.type == 'recently-viewed-products') {
        var object = {
