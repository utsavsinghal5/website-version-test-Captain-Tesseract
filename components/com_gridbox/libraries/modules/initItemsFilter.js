...
var priceRange = function(element) {
    ...
    this.step = 1;
    this.value = new Array(element.dataset.minValue, element.dataset.maxValue);
...
function getItemsFilterCount(obj, wrapper, target)
{
    ...
    if (obj.auto) {
        ...
        $g.ajax({
            type: "POST",
            dataType: 'text',
            url: "index.php?option=com_gridbox&task=page.getItemsFilterCount",
            data: {
                app: obj.app,
...
app.initItemsFilter = function(obj, key){
    ...
    $g('#'+key).on('click', '.ba-selected-filter-values i.zmdi-close', function(){
    ...
    $g('#'+key).on('click', '.ba-selected-filter-values-remove-all', function(){
        ...
        $g(wrapper).find('input[name][type="number"][data-min]').each(function(){
            let max = wrapper.querySelector('input[name="'+this.name+'"][data-max]'),
                data = $g(this).closest('.ba-field-filter-value-wrapper').find('.price-range-track').data('priceRange');
