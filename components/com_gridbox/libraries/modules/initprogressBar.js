...
function startProgressBar(obj, $this)
{
    ...
    bar.stop().css('width', '0%').animate({
        ...
        step: function(now, fx) {
            $this.numberBar.textContent = Math.floor(now)+'%';
        },
        complete: function(){
            $this.numberBar.textContent = obj.target+'%';
        }
    });
}
...
app.initprogressBar = function(obj, key){
    ...
    $g(window).off('scroll.progress-bar-'+key).on('scroll.progress-bar-'+key, $g.proxy(compareProgressBarPosition, $this, $this));
