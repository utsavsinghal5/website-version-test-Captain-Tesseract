...
class gridboxModelComments extends JModelItem
{
    ...
    public function sendReplyEmail($id)
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->leftJoin('`#__gridbox_comments` AS pc ON '.$db->quoteName('pc.id').' = '.$db->quoteName('c.parent'));
        $db->setQuery($query);
        $data = $db->loadObject();
        ...
        if ($data->user_notification == 0 && !empty($data->recipient) && $flag) {
            ...
            if (empty($data->avatar)) {
                $avatar = gridboxHelper::getCommentsUserAvatar($data->email);
            } else {
                $avatar = $data->avatar;
            }
            ...
            $subject = JText::_('NEW_REPLY_TO_COMMENT_ON').' '.$data->title;
            $mailer->isHTML(true);
            $mailer->Encoding = 'base64';
            include(JPATH_ROOT.'/components/com_gridbox/views/layout/patterns/comments-box/comments-box-reply-email-pattern.php');
    ...
    public function sendModeratorEmail($id)
    {
        ...
        if (!empty($moderators) && $data->admin_notification == 0) {
            ...
            if (!empty($recipients)) {
                ...
                $message = str_replace("\n", '<br>', $data->message);
                $date = gridboxHelper::getPostDate($data->date);
                $subject = JText::_('NEW_COMMENT_POSTED_ON').' '.$data->title;
                $mailer->isHTML(true);
                $mailer->Encoding = 'base64';
                include(JPATH_ROOT.'/components/com_gridbox/views/layout/patterns/comments-box/comments-box-moderator-email-pattern.php');
                $mailer->setSender($sender);
                $mailer->setSubject($subject);
                $mailer->addRecipient($recipients);
                $mailer->setBody($out);
    ...
    public function sendReportEmail($id)
    {
        ...
        if (!empty($moderators)) {
            ...
            if (!empty($recipients)) {
                ...
                $mailer->isHTML(true);
                $mailer->Encoding = 'base64';
                include(JPATH_ROOT.'/components/com_gridbox/views/layout/patterns/comments-box/comments-box-report-email-pattern.php');
                $mailer->setSender($sender);
                $mailer->setSubject($subject);
                $mailer->addRecipient($recipients);
                $mailer->setBody($out);
