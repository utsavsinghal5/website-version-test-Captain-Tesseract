...
class gridboxModelGridbox extends JModelItem
{
    ...
    public function getAppLayout()
    {
        ...
        if (empty($item->app_layout)) {
            $item->app_layout = JFile::read(JPATH_ROOT.'/components/com_gridbox/views/layout/apps/'.$item->type.'/app.html');
        }
    ...
    public function createPage()
    {
        ...
        if (!empty($type) && $type != 'single') {
            include JPATH_ROOT.'/components/com_gridbox/views/layout/apps/'.$type.'/post.php';
        } else {
        ...
        $array = array('title' => $title, 'page_alias' => $alias, 'page_category' => $category,
