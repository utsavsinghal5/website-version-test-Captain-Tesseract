...
?>
<link rel="stylesheet" type="text/css" href="<?php echo JUri::root(); ?>administrator/components/com_gridbox/assets/css/ba-admin.css">
...
foreach ($this->item->products as $product) {
    ...
    $priceText = $this->preparePrice($price, $this->item->currency_symbol, $this->item->currency_position);
    $info = array();
    foreach ($product->variations as $variation) {
        $info[] = '<span>'.$variation->title.' '.$variation->value.'</span>';
    }
    $infoStr = implode('<span>/</span>', $info);
    $amountText = $this->preparePrice($amount, $this->item->currency_symbol, $this->item->currency_position);
    $data = array($product->title, $infoStr, $product->quantity);
    if (!empty($this->item->tax) && $this->item->tax_mode == 'incl') {
        $netPrice = $price - ($price - $price / ($product->tax_rate / 100 + 1));
        $netText = $this->preparePrice($netPrice, $this->item->currency_symbol, $this->item->currency_position);
        $data[] = $netText;
    }
    $data[] = $priceText;
    $data[] = $amountText;
...
<?php
                    foreach ($data as $i => $v) {
                        if ($i == 1) {
                            ...
                            $str = $this->preparePrice($price, $this->item->currency_symbol, $this->item->currency_position);
                        } else {
                            $str = '';
                        }
?>
...
<?php
$price = $this->preparePrice($this->item->subtotal, $this->item->currency_symbol, $this->item->currency_position);
?>
...
<?php
if ($this->item->promo) {
    $price = $this->preparePrice($this->item->promo->value, $this->item->currency_symbol, $this->item->currency_position);
?>
...
if ($this->item->shipping) {
    $price = $this->preparePrice($this->item->shipping->price, $this->item->currency_symbol, $this->item->currency_position);
...
if ($this->item->shipping && !empty($this->item->shipping->tax) && $this->item->tax_mode == 'incl') {
    $price = $this->preparePrice($this->item->shipping->tax, $this->item->currency_symbol, $this->item->currency_position);
...
if (!empty($this->item->tax) && $this->item->tax_mode == 'excl' && $taxCount == 0) {
    $price = $this->preparePrice($this->item->tax, $this->item->currency_symbol, $this->item->currency_position);
?>
<div style="margin-bottom: 4mm;">
...
<?php
    ...
    foreach ($taxes as $tax) {
        $price = $this->preparePrice($tax->amount, $this->item->currency_symbol, $this->item->currency_position);
?>
<div style="margin-bottom: 4mm;">
...
?>
<?php
$price = $this->preparePrice($this->item->total, $this->item->currency_symbol, $this->item->currency_position);
?>
