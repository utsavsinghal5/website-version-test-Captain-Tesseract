...
<?php
                for ($i = 1; $i < 6; $i++) {
?>
                    <i class="zmdi zmdi-star<?php echo $i <= $comment->rating ? ' active' : ''; ?>"
...
<?php
        if ($comment->parent != 0) {
?>
            <span class="comment-reply-name"><i class="zmdi zmdi-mail-reply"></i><?php echo $replyName; ?></span>
<?php
        }
?>
            </div>
...
<?php
                if (!empty($user) && $user->type == $comment->user_type && $user->type != 'guest' && $user->id == $comment->user_id) {
?>
                    <i class="zmdi zmdi-close delete-comment-attachment-file" data-id="<?php echo $file->id; ?>"
...
<?php
                if (!empty($user) && $user->type == $comment->user_type && $user->type != 'guest' && $user->id == $comment->user_id) {
?>
                    <i class="zmdi zmdi-delete delete-comment-attachment-file" data-id="<?php echo $file->id; ?>"
