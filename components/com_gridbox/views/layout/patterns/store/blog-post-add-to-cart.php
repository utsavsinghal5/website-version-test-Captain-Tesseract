...
if (isset($data->default)) {
    $prices = self::prepareProductPrices($page->id, $data->default->price, $data->default->sale_price);
} else {
    $prices = self::prepareProductPrices($page->id, $data->price, $data->sale_price);
}
$price = $prices->regular;
$variations_map = self::getProductVariationsMap($page->id);
if (isset($data->default)) {
...
?>
        <span class="ba-blog-post-add-to-cart<?php echo $stock; ?>"><?php echo JText::_($text); ?></span>
    </div>
<?php
    }
?>
