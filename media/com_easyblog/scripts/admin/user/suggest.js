EasyBlog.module('admin/user/suggest', function($) {
...
EasyBlog.Controller('User.Suggest', {
	defaultOptions: {
		max: null,

		// this option determine for whether you allow highlighted those unrelated string
		exclusive: false,
		exclusion: [],
		minLength: 1,
		highlight: true,
		...
		"query": {
			"suggest": "admin/views/bloggers/suggest"
		},
	...
	"{self} filterItem": function(el, event, item) {
		...
		var title = html.find('[data-user-name]').text();
		var id = html.find('[data-user-id]').val();
