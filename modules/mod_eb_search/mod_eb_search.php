...
if ($showLocation)
{
	...
	$lists['location_id'] = JHtml::_('select.genericlist', $options, 'location_id', ' class="inputbox location_box' . $bootstrapHelper->getFrameworkClass('form-control', 1) . '"', 'id', 'name', $locationId);
}
...
if ($enableRadiusSearch)
{
	...
	{
		$options[] = JHtml::_('select.option', (int) $option, JText::sprintf($languageKey, $option));
	}

	$lists['filter_distance'] = JHtml::_('select.genericlist', $options, 'filter_distance', ' class="inputbox location_box' . $bootstrapHelper->getFrameworkClass('form-control', 1) . '" ', 'value', 'text', $filterDistance);
}
