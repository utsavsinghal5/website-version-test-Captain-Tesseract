...
class plgEventBookingTicketTypes extends JPlugin
{
	...
	private function storeTicketTypeForChildrenEvents($row, $ticketTypeIds, $isNew, $hasMultipleTicketTypes)
	{
		...
		if ($isNew)
		{
			...
			{
				$sql = 'INSERT INTO #__eb_ticket_types (event_id, title, discount_rules, description, price, capacity, weight, max_tickets_per_booking, publish_up, publish_down, ordering, parent_ticket_type_id)'
					. " SELECT $childEventId, title, discount_rules, description, price, capacity, weight, max_tickets_per_booking, publish_up, publish_down, ordering, id FROM #__eb_ticket_types WHERE event_id = $row->id";
				$db->setQuery($sql);
				$db->execute();
			}
		}
				...
				foreach ($ticketTypeIds as $ticketTypeId)
				{
					...
					else
					{
						...
						$query->clear()
							...
							->columns('event_id, title, discount_rules, description, price, capacity, weight, max_tickets_per_booking, publish_up, publish_down, ordering, parent_ticket_type_id')
							->values("$childEventId, $title, $discountRules, $description ,$price, $capacity, $weight, $maxTicketsPerBooking, $publishUp, $publishDown, $ordering, $rowParentTicketType->id");
						$db->setQuery($query)
                            ->execute();
					}
				}
